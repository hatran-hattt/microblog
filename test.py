...
class TestPostModel(unittest.TestCase):
    ...
    def test_get_posts(self):
        ...
        p1 = Post(
            content="post from john", author=u1, timestamp=now + timedelta(seconds=1)
        )
        p2 = Post(
            content="post from susan", author=u2, timestamp=now + timedelta(seconds=4)
        )
        p3 = Post(
            content="post from mary", author=u3, timestamp=now + timedelta(seconds=3)
        )
        p4 = Post(
            content="post from david", author=u4, timestamp=now + timedelta(seconds=2)
        )
